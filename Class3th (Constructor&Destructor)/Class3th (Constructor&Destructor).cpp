...
public:
	...
	Card(const Card& clone) // 레퍼런스가 안들어가면 무한루프에 빠짐
	{
		name = clone.name;
		grade = clone.grade;
...
#pragma region 얕은 복사
	// 객체를 복사할 때 주소 값을 복사하여 같은 메모리 공간을
	// 가리키게 하는 복사.

	// 얕은 복사의 경우 같은 객체가 서로 같은 메모리 공간을 참조하고
	// 있기 때문에 하나의 객체로 값을 변경하게 되면 서로 참조된 객체도
	// 함께 영향을 받음.
...
#pragma region 깊은 복사
	// 객체를 복사할 때 참조 값이 아닌 객체 자체로 새로 복사하여 서로 다른
	// 메모리를 생성하는 복사.
